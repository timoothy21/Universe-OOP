...
public class user_Enrolled_1_ViewEventUI_fix extends JFrame {
	 ...
	 * Create the frame.
	 */
	public user_Enrolled_1_ViewEventUI_fix(User users) {
		...
		JLabel lblFullName = new JLabel(users.getFullName());
		...
		JLabel lblUsername = new JLabel(users.getUsername());
		...
		JLabel lblEmail = new JLabel(users.getEmail());
		...
		JLabel lbl_user_role = new JLabel("USER");
		...
		scrollPane.setBounds(134, 55, 422, 290);
...
//		scrollPane.setViewportView(table);
		...
		table = new JTable();
		model = connection.showEventEnrolled(table, users);
...
//				System.out.println(model.getColumnCount());
				...
				for(int j = 0; j < model.getColumnCount(); j++) {
					result[j] = (String) model.getValueAt(i, j);
//							table.getModel().getValueAt(modelIndex, j);
